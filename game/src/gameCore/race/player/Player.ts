...
export default class Player {
	private _id: string;
	private helmetIndex: number | null;
	...
	public setWorldPosition(p: Point) {
	...
	public getHelmetIndex(): number {
		return this.helmetIndex ? this.helmetIndex : 0;
	}
	...
	public getLastPassedCheckPoint(): number {
		return this.lastValidCheckpoint;
	}

	public passingByCheckpoint(checkpointGroup: number): void {
		if (checkpointGroup > this.lastValidCheckpoint) this.lastValidCheckpoint = checkpointGroup;
	}
	...
	public setMaxMovementDistance(moveDistance: number): void {
	...
	public answeredQuestion(isAnswerCorrect: boolean): void {
		if (isAnswerCorrect) {
			if (this.getCurrentStatus() !== StatusType.BananaStatus) {
